...
func getPodsToPreemptByDistance(pods []*v1.Pod, requirements admissionRequirementList) ([]*v1.Pod, error) {
	...
	// 一直循环查找需要驱逐的Pod，直到这些Pod 满足requirements的请求
	for len(requirements) > 0 {
		...
		// 去除当前该驱逐的Pod后，还需要释放的资源，然后进行下一轮的Pod选举
